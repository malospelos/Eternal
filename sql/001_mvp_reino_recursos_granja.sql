...
USING (VALUES
    ('FOOD',  N'Alimentos', 0, 1, 1),
    ('WOOD',  N'Madera',    0, 1, 2),
    ('STONE', N'Piedra',    0, 1, 3),
    ('GOLD',  N'Oro',       0, 1, 4),
    ('GEMS',  N'Cristales', 1, 0, 5)
) AS S(Code, Name, IsPremium, IsLootable, SortOrder)
ON T.Code = S.Code
WHEN MATCHED THEN UPDATE SET
    Name = S.Name,
    IsPremium = S.IsPremium,
    IsLootable = S.IsLootable,
...
BEGIN
    ...
    CREATE INDEX IX_ec_resource_transactions_KingdomDate
        ON dbo.ec_resource_transactions(KingdomId, CreatedAt DESC);
END;
...
USING (VALUES
    ('CASTLE', N'Castillo', 0, NULL,   10, 1),
    ('FARM',   N'Granja',   1, 'FOOD', 10, 2)
) AS S(Code, Name, IsResourceProducer, ProductionResourceCode, MaxLevel, SortOrder)
ON T.Code = S.Code
WHEN MATCHED THEN UPDATE SET
    Name = S.Name,
    IsResourceProducer = S.IsResourceProducer,
    ProductionResourceCode = S.ProductionResourceCode,
    MaxLevel = S.MaxLevel,
    SortOrder = S.SortOrder
...
BEGIN
    ...
    (
        BuildingTypeCode    VARCHAR(30) NOT NULL,
        Level               INT NOT NULL,
        BuildSeconds        INT NOT NULL,
...
USING (VALUES
    ('CASTLE',1,0,    CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',2,10,   CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',3,30,   CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',4,60,   CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',5,180,  CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',6,480,  CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',7,900,  CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',8,1800, CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',9,3600, CAST(NULL AS DECIMAL(19,4)),1),
    ('CASTLE',10,7200,CAST(NULL AS DECIMAL(19,4)),1),
    ('FARM',1,5,      CAST(600 AS DECIMAL(19,4)),1),
    ('FARM',2,10,     CAST(900 AS DECIMAL(19,4)),1),
    ('FARM',3,30,     CAST(1300 AS DECIMAL(19,4)),2),
    ('FARM',4,60,     CAST(1850 AS DECIMAL(19,4)),3),
    ('FARM',5,180,    CAST(2600 AS DECIMAL(19,4)),4),
    ('FARM',6,480,    CAST(3600 AS DECIMAL(19,4)),5),
    ('FARM',7,900,    CAST(4900 AS DECIMAL(19,4)),6),
    ('FARM',8,1800,   CAST(6500 AS DECIMAL(19,4)),7),
    ('FARM',9,3600,   CAST(8500 AS DECIMAL(19,4)),8),
    ('FARM',10,7200,  CAST(11000 AS DECIMAL(19,4)),9)
...
WHEN MATCHED THEN UPDATE SET
    ...
    RequiredCastleLevel = S.RequiredCastleLevel
...
BEGIN
    ...
    (
        ...
        CONSTRAINT FK_ec_building_level_costs_Level FOREIGN KEY (BuildingTypeCode, Level)
            REFERENCES dbo.ec_building_levels(BuildingTypeCode, Level),
        CONSTRAINT FK_ec_building_level_costs_Resource FOREIGN KEY (ResourceCode)
            REFERENCES dbo.ec_resource_types(Code)
    );
END;
...
WHEN NOT MATCHED THEN INSERT (BuildingTypeCode, Level, ResourceCode, Amount)
VALUES (S.BuildingTypeCode, S.Level, S.ResourceCode, S.Amount);
...
BEGIN
    ...
    (
        ...
        KingdomId           BIGINT NOT NULL,
        KingdomBuildingId   BIGINT NOT NULL,
        BuilderSlot         INT NOT NULL,
        FromLevel           INT NOT NULL,
        ToLevel             INT NOT NULL,
        StartedAt           DATETIME2(3) NOT NULL,
        FinishAt            DATETIME2(3) NOT NULL,
        CompletedAt         DATETIME2(3) NULL,
        Status              VARCHAR(20) NOT NULL CONSTRAINT DF_ec_construction_queue_Status DEFAULT 'ACTIVE',
...
/* ---------- Datos demo para el primer hito ---------- */
IF NOT EXISTS (SELECT 1 FROM dbo.ec_players WHERE PlayerName = N'Comandante')
BEGIN
    INSERT INTO dbo.ec_players(PlayerName) VALUES (N'Comandante');

    DECLARE @PlayerId BIGINT = SCOPE_IDENTITY();
    INSERT INTO dbo.ec_kingdoms(PlayerId, KingdomName) VALUES (@PlayerId, N'Reino de Elyndor');

    DECLARE @KingdomId BIGINT = SCOPE_IDENTITY();

    INSERT INTO dbo.ec_kingdom_resources(KingdomId, ResourceCode, Amount)
    VALUES
        (@KingdomId, 'FOOD',  500),
        (@KingdomId, 'WOOD',  500),
        (@KingdomId, 'STONE', 500),
    ...
    INSERT INTO dbo.ec_kingdom_buildings(KingdomId, BuildingTypeCode, Level, PositionX, PositionY)
    VALUES
        (@KingdomId, 'CASTLE', 1, 0, 0),
        (@KingdomId, 'FARM',   0, 1, 1);
END;
