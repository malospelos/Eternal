...
try {
  ...
  for (let phase = 1; phase <= 7; phase++) {
    ...
    await page.locator('.phase-complete').filter({ hasText: `FASE ${phase} COMPLETADA` }).waitFor();
  ...
  await page.reload({ waitUntil: 'networkidle' });
  await page.locator('.phase-complete').filter({ hasText: 'FASE 7 COMPLETADA' }).waitFor();
