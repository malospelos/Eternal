...
export class ResearchTree { private points=1000;private done=new Set<string>();list(){return NODES.map(n=>({...n,unlocked:this.done.has(n.id),available:!n.requires||this.done.has(n.requires)}))}availablePoints(){return this.points}research(id:string){const n=NODES.find(x=>x.id===id);if(!n)throw new Error('Investigación desconocida');if(this.done.has(id))return;if(n.requires&&!this.done.has(n.requires))throw new Error('Falta investigación previa');if(this.points<n.cost)throw new Error('Conocimiento insuficiente');this.points-=n.cost;this.done.add(id)}completed(){return this.done.size}isComplete(){return this.done.size===NODES.length}}
